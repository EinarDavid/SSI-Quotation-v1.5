...
import { getQuotationCant, postFilters } from "../services/cotizacionService";
  ...
  const DescargarReporte = () => {
    ...
    binary_univers.forEach((element, index) => {
      listaDescargable.push({
        ...
        date: element.date,
        total_effort: element.total_effort,
        total_effort_approved: element.total_effort_approved,
        link_jira: element.link_jira,
      });
    });
                ...
                <table className="tableContainer">
                  <thead>
                    <tr>
                      ...
                      <th className="titleTable">T. Hrs estimado</th>
                      <th className="titleTable">T. Hrs SOW aprobado</th>
                    </tr>
                  </thead>
                  {quotation?.map((quo, i) => (
                    <tbody key={quo.id_quotation}>
                      <tr>
                        <td>
                          {quo.status === "NW" || quo.status === "NEW" ? (
                            <>
                              <button
                                className="buttonPrint"
                                onClick={() =>
                                  navigate("/new/quotation/" + quo.id_quotation)
                                }
                              >
                                <img
                                  src={Images.VIEW}
                            ...
                            <button
                              className="buttonPrint"
                              onClick={() =>
                                navigate("/inprogress/quotation/" + quo.id_quotation)
                              }
                            >
                            ...
                            quo.status === "BLOCKED" || quo.status === "SOW APROBADO" ? (
                            <>
                              <button
                                className="buttonPrint"
                                onClick={() =>
                                  navigate(
                                    "/registered/quotation/" + quo.id_quotation
                                  )
                                }
                              >
                                <img
